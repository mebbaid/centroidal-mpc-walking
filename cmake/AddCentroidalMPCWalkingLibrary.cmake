# Copyright (C) 2019 Istituto Italiano di Tecnologia (IIT). All rights reserved.
# This software may be modified and distributed under the terms of the
# GNU Lesser General Public License v2.1 or any later version.

function(add_centroidal_mpc_walking_library)
  # Parse arguments
  set(options)
  set(oneValueArgs NAME)
  set(multiValueArgs SOURCES HEADERS LINK_LIBRARIES SUBDIRECTORIES)

  set(prefix "centroidal_mpc_walking_library")

  cmake_parse_arguments(${prefix}
    "${options}"
    "${oneValueArgs}"
    "${multiValueArgs}"
    ${ARGN})

  # Set variables
  set(name ${${prefix}_NAME})
  set(sources ${${prefix}_SOURCES})
  set(headers ${${prefix}_HEADERS})
  set(link_libraries ${${prefix}_LINK_LIBRARIES})
  set(subdirectories ${${prefix}_SUBDIRECTORIES})

  # Add library target to the project
  add_library(${name} ${sources} ${headers})

  # Add C++17 features
  target_compile_features(${name} PRIVATE cxx_std_17)
  target_compile_definitions(${name} PRIVATE -D_USE_MATH_DEFINES)

  # Set target properties
  set_target_properties(${name} PROPERTIES
      OUTPUT_NAME "stable-mpc-${name}"
      VERSION ${StableMPCWalking_VERSION})

  # Link libraries
  target_link_libraries(${name} PRIVATE ${link_libraries})

  # Include directories
  target_include_directories(${name}
    PRIVATE "$<BUILD_INTERFACE:${CMAKE_CURRENT_SOURCE_DIR}/include>")

  # Specify installation targets, typology, and destination folders
  install(TARGETS ${name}
    DESTINATION "${CMAKE_INSTALL_LIBDIR}" COMPONENT bin)

  # Add subdirectories
  foreach(subdir ${subdirectories})
    add_subdirectory(${subdir})
  endforeach()

  message(STATUS "Created target library ${name}.")
endfunction()
